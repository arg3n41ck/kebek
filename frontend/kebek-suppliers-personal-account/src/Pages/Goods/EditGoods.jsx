...
function EditGoods() {
    ...
    const { postProducts, elevators, getElevators, getProductTypes, productTypes, getProductById, productById, patchProduct } = React.useContext(goodsContext)
    const { id } = useParams()
    const formRef = React.useRef(null);

    React.useEffect(() => {
        formRef.current.setValues({
            elevator: !productById ? "default" : productById.elevator.id,
            type: !productById ? "default" : productById.type.id,
            set_number: !productById ? '' : productById.setNumber,
            price: !productById ? '' : productById.price,
            min_limit: !productById ? '' : productById.minLimit,
            max_limit: !productById ? '' : productById.maxLimit,
            residue: !productById ? '' : productById.residue,
            image: !productById ? '' : productById.image
        })

        return () => {
            formRef.current.setValues(null)
        }
    }, [productById]);
    ...
    const handleSubmit = async (values, resetForm, id) => {
        ...
        await patchProduct(id, formData).then(() => {
            resetForm()
            setAvatarPreview([])
        })
    }
    ...
    if (!productById?.id && (!productTypes.length && !elevators)) {
        return <Loader />
    }
        ...
        <div className={classes.addStaff_container}>
            ...
            >
                ...
                <ul className={"d-flex align-items-center m-0 p-0 ps-2"} style={{ listStyle: "none" }}>
                    ...
                    <li style={{ margin: '0 0 0 10px', fontWeight: 600 }}>/ {locale === "ru" ? "Изменить товары" : "Өнімдерді қосу"}</li>
                </ul>
            </Box>
            <div className={classes.addStaff_container__items}>
                <div className={classes.form1}>
                    <Formik
                        ...
                        onSubmit={(values, { resetForm }) => handleSubmit(values, resetForm, id)}
                        ...
                        {({
                            ...
                            return (
                                <Form>
                                    ...
                                    />
                                    <Button type="submit" sx={{ maxWidth: 281, height: 53, width: "100%", fontSize: 18, fontWeight: 600, textTransform: "none" }} color="success" variant="contained" >Изменить товар</Button>
                                </Form>
                            );
                        }}
                    </Formik>
...
export function ImageDropzone({
    ...
    return (
        ...
        >
            ...
            {(!!values.image && !avatarPreview) && < img style={{ width: 64, height: 54, objectFit: "cover" }} src={values.image} alt="image" />}

            {!!avatarPreview && <Item data={avatarPreview} />}
